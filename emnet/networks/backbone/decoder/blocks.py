...
class MinkowskiInverseSparseBottleneckV2(nn.Module):
    def __init__(
        ...
        if act_layer == ME.MinkowskiReLU:
            act_layer = partial(act_layer, inplace=True)  # type: ignore[reportAssignmentType]
        ...
        if block_index == 0:
            if self.out_reduction < in_reduction:
                ...
                self.upsample = ME.MinkowskiConvolutionTranspose(
                    ...
                    kernel_size=3,
                    bias=bias,
                    stride=in_reduction // self.out_reduction,
                    dimension=dimension,
                )
            else:
                self.upsample = None
    ...
    def reset_parameters(self):
        if self.upsample is not None and hasattr(self.upsample, "reset_parameters"):
        ...
        for block_index in range(depth):
            ...
            self.blocks.add_module(
                ...
                MinkowskiInverseSparseBottleneckV2(
                    ...
                    bias=bias,
                    dimension=dimension,
                    act_layer=get_me_layer(
                        act_layer
                    ),  # pyright: ignore[reportArgumentType]
                    norm_layer=get_me_layer(
                        norm_layer
                    ),  # pyright: ignore[reportArgumentType]
                ),
            )
