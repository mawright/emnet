...
from emnet.config.criterion import MatcherConfig
...
def get_class_cost(is_electron_logit: Tensor, batch_offsets: Tensor) -> list[Tensor]:
    ...
    batch_losses = split_batch_concatenated_tensor(loss, batch_offsets)
...
def get_bce_cost(
    ...
    pos_tensor: Tensor = sparse_flatten(
    ...
    neg_tensor: Tensor = sparse_flatten(
        torch.sparse_coo_tensor(
    ...
    true_segmap_binarized: Tensor = sparse_flatten(
        torch.sparse_coo_tensor(
            segmap.indices(),
...
def get_dice_cost(
    ...
    portions_flat: Tensor = sparse_flatten(portions, 1, 2)
    true_portions_flat: Tensor = sparse_flatten(true_portions, 1, 2)
...
def get_nll_distance_cost(
    ...
    predicted_pos_per_image = split_batch_concatenated_tensor(
        predicted_positions, query_batch_offsets
    )
    predicted_std_dev_per_image = split_batch_concatenated_tensor(
        predicted_std_dev_cholesky, query_batch_offsets
    )
    true_pos_per_image = split_batch_concatenated_tensor(
...
def get_huber_distance_cost(
    ...
    predicted_batches = split_batch_concatenated_tensor(
        predicted_positions, predicted_batch_offsets
    )
    true_batches = split_batch_concatenated_tensor(true_positions, true_batch_offsets)
...
def get_box_l1_cost(
    ...
    true_batches = split_batch_concatenated_tensor(true_boxes_xyxy, true_batch_offsets)
